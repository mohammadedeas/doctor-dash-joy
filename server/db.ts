...
// In production (Electron), DB_DIR is set to app.getPath("userData")
// so the database lives in %APPDATA%/Clinic OS/ (writable).
// For web hosting, it defaults to the project root where the server is started.
const dbDir = process.env.DB_DIR || process.cwd();
const DB_PATH = path.join(dbDir, "clinic.db");
